Shader "Unlit/GratingWithBlinnPhong"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 BlinnPhong(float3 normal, float3 lightDir, float3 viewDir, float3 lightColor, float shininess)
            {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float3 envcolor1 = texCUBElod(_Cube, float4(reflectiondir1, _Roughness * 6.0)).rgb;
                float3 envcolor2 = texCUBElod(_Cube, float4(reflectiondir2, _Roughness * 6.0)).rgb;

                //--------------------------
                // Blinn-Phong Lighting
                //
                float shininess = 32.0; // Adjust the shininess factor as needed.
                float3 shadingNear = BlinnPhong(normal_grate1, lightdir, viewdir, lightcolor, shininess);
                float3 shadingFar = BlinnPhong(normal_grate2, lightdir, viewdir, lightcolor, shininess);
